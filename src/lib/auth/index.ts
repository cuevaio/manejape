...
import { SESSION_TOKEN_COOKIE_EXPIRES_IN_S, SESSION_TOKEN_COOKIE_NAME } from './constants';
...
export async function validateSessionToken(token: string): Promise<Session | null> {
	...
	if (Date.now() >= session.expiresAt.getTime() - 1000 * SESSION_TOKEN_COOKIE_EXPIRES_IN_S / 2) { // 15 days
		await redis.hset(
			`session:${session.id}`,
      {
